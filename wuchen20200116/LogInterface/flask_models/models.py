# coding: utf-8
from sqlalchemy import Column, DateTime, Float, ForeignKey, Index, Integer, SmallInteger, String
...
class ClientBroker(db.Model):
    __tablename__ = 'client_broker'

    client = db.Column(db.ForeignKey('auth_user.id', ondelete='RESTRICT', onupdate='RESTRICT'), primary_key=True, nullable=False)
    broker = db.Column(db.ForeignKey('broker.id'), primary_key=True, nullable=False, index=True)
    paid = db.Column(db.Integer, nullable=False)

    broker1 = db.relationship('Broker', primaryjoin='ClientBroker.broker == Broker.id', backref='client_brokers')
    auth_user = db.relationship('AuthUser', primaryjoin='ClientBroker.client == AuthUser.id', backref='client_brokers')
